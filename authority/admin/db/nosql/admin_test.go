...
func Test_unmarshalDBAdmin(t *testing.T) {
	adminID := "adminID"
	type test struct {
		in       []byte
		err      error
	...
	for name, run := range tests {
		tc := run(t)
		t.Run(name, func(t *testing.T) {
			if dba, err := unmarshalDBAdmin(tc.in, adminID); err != nil {
				switch k := err.(type) {
				case *admin.Error:
					if assert.NotNil(t, tc.adminErr) {
						assert.Equals(t, k.Type, tc.adminErr.Type)
...
func Test_unmarshalAdmin(t *testing.T) {
	adminID := "adminID"
	type test struct {
		in       []byte
		err      error
	...
	for name, run := range tests {
		tc := run(t)
		t.Run(name, func(t *testing.T) {
			if adm, err := unmarshalAdmin(tc.in, adminID); err != nil {
				switch k := err.(type) {
				case *admin.Error:
					if assert.NotNil(t, tc.adminErr) {
						assert.Equals(t, k.Type, tc.adminErr.Type)
